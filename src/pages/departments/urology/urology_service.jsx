...
// The API endpoint for fetching urology services
const API_URL = "API/departments/urology/urology_service.php";
...
const getImageUrl = (imagePath) => {
  ...
  return `API/departments/urology/${imagePath}`;
};
...
const ServiceItem = ({ service, onClick, currentLanguage, index }) => {
  ...
  const isSelected =
    sessionStorage.getItem("urology_service_last_selected") === String(index);
  const sectionNumber = Math.min(index + 1, 10);
  const sectionRoute = `/urology_section${sectionNumber}`;
...
const UrologyService = ({ currentLanguage }) => {
  ...
  const content = {
    en: {
      ...
      subtitle:
        "Comprehensive urological care with advanced diagnostics and treatments",
      ...
      cta: {
        ...
        description: "Schedule a consultation with our expert urology team",
        paragraph: "Schedule a consultation with our expert urology team",
    ...
    km: {
      title: "សេវាកម្មជំងឺផ្លេកខាងក្នុង",
      subtitle:
        "ការថែទាំជំងឺផ្លេកខាងក្នុងទូលំទូលាយជាមួយការធ្វើរោគវិនិច្ឆ័យ និងការព្យាបាលឆ្នើម",
      tagline: "ការថែទាំជំងឺផ្លេកខាងក្នុងដោយអ្នកជំនាញ",
      stats: {
      ...
      cta: {
        ...
        description: "កក់ណាត់ពិគ្រោះជាមួយក្រុមអ្នកជំនា៍ជំងឺផ្លេកខាងក្នុងរបស់យើង",
        paragraph: "កក់ណាត់ពិគ្រោះជាមួយក្រុមអ្នកជំនាញជំងឺផ្លេកខាងក្នុងរបស់យើង",
        button: "កក់ណាត់",
      },
      error: {
  ...
  useEffect(() => {
    const fetchServices = async () => {
      ...
      try {
        ...
        console.error("Error fetching urology services:", err);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-16 md:mb-20 max-w-6xl">
            ...
            >
                       {" "}
              {currentLanguage === "en"
                ? "The Urology Department located in Khmer-Soviet Friendship Hospital, provides a wide range of services for urinary tract and male reproductive system health, from diagnostic procedures to advanced surgical treatments. Our comprehensive urology services include:."
                : "នាយកដ្ឋានជំងឺផ្លេកខាងក្នុង មានទីតាំងនៅក្នុងមន្ទីរពេទ្យមិត្តភាពខ្មែរ-សូវៀត ផ្តល់សេវាកម្មយ៉ាងទូលំទូលាយសម្រាប់សុខភាពផ្លូវទឹកនោម និងប្រព័ន្ធបង្កើតកូនរបស់បុរស តាំងពីរបៀបធ្វើរោគវិនិច្ឆ័យ ដល់ការព្យាបាលតាមរយៈការវះកាត់កម្រិតខ្ពស់។ សេវាកម្មជំងឺផ្លេកខាងក្នុងទូលំទូលាយរបស់យើងរួមមាន:"}
                     {" "}
            </p>
