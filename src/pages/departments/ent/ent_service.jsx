...
// The API endpoint for fetching ENT services
const API_URL = "API/departments/ent/ent_service.php";
...
const getImageUrl = (imagePath) => {
  ...
  return `API/departments/ent/${imagePath}`;
};
...
const ServiceItem = ({ service, onClick, currentLanguage, index }) => {
  ...
  const isSelected =
    sessionStorage.getItem("ent_service_last_selected") === String(index);
  const sectionNumber = Math.min(index + 1, 10);
  const sectionRoute = `/ent_section${sectionNumber}`;
...
const EntService = ({ currentLanguage }) => {
  ...
  const content = {
    en: {
      title: "ENT Services",
      subtitle:
        "Comprehensive ear, nose, and throat care with advanced diagnostics and treatments",
      ...
      cta: {
        ...
        description: "Schedule a consultation with our expert ENT team",
        paragraph: "Schedule a consultation with our expert ENT team",
    ...
    km: {
      title: "សេវាកម្មត្រចៀក ច្រមុះ និងក",
      subtitle:
        "ការថែទាំត្រចៀក ច្រមុះ និងកទូលំទូលាយជាមួយការធ្វើរោគវិនិច្ឆ័យ និងការព្យាបាលឆ្នើម",
      tagline: "ការថែទាំត្រចៀក ច្រមុះ និងកដោយអ្នកជំនាញ",
      stats: {
      ...
      cta: {
        ...
        description: "កក់ណាត់ពិគ្រោះជាមួយក្រុមអ្នកជំនា៍ត្រចៀក ច្រមុះ និងករបស់យើង",
        paragraph: "កក់ណាត់ពិគ្រោះជាមួយក្រុមអ្នកជំនាញត្រចៀក ច្រមុះ និងករបស់យើង",
        button: "កក់ណាត់",
      },
      error: {
  ...
  useEffect(() => {
    const fetchServices = async () => {
      ...
      try {
        ...
        console.error("Error fetching ENT services:", err);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-16 md:mb-20 max-w-6xl">
            ...
            >
                       {" "}
              {currentLanguage === "en"
                ? "The ENT Department located in Khmer-Soviet Friendship Hospital, provides a wide range of services for ear, nose, and throat conditions, from diagnostic procedures to advanced surgical treatments. Our comprehensive ENT services include:."
                : "នាយកដ្ឋានត្រចៀក ច្រមុះ និងក មានទីតាំងនៅក្នុងមន្ទីរពេទ្យមិត្តភាពខ្មែរ-សូវៀត ផ្តល់សេវាកម្មយ៉ាងទូលំទូលាយសម្រាប់ជំងឺត្រចៀក ច្រមុះ និងក តាំងពីរបៀបធ្វើរោគវិនិច្ឆ័យ ដល់ការព្យាបាលតាមរយៈការវះកាត់កម្រិតខ្ពស់។ សេវាកម្មត្រចៀក ច្រមុះ និងកទូលំទូលាយរបស់យើងរួមមាន:"}
                     {" "}
            </p>
