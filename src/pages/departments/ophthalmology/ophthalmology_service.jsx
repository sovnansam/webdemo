...
// The API endpoint for fetching Ophthalmology services
const API_URL = "API/departments/optamology/optamology_service.php";
...
const ServiceItem = ({ service, onClick, currentLanguage, index }) => {
  ...
  const isSelected =
    sessionStorage.getItem("ophthalmology_service_last_selected") === String(index);
  const sectionNumber = Math.min(index + 1, 10);
  const sectionRoute = `/ophthalmology_section${sectionNumber}`;
  ...
  return (
    <motion.div
      ...
      id={`ophthalmology-service-item-${index}`}
      tabIndex={0}
                ...
                <motion.button
                  ...
                  onClick={() => {
                    sessionStorage.setItem(
                      "ophthalmology_service_last_selected",
                      String(index)
                    );
...
const OphthalmologyService = ({ currentLanguage }) => {
  ...
  const content = {
    en: {
      ...
      subtitle:
        "Comprehensive eye care with advanced diagnostics and treatments",
      tagline: "Expert Eye Care",
      stats: {
      ...
      cta: {
        title: "Your Vision Deserves The Best",
        description: "Schedule a consultation with our expert ophthalmology team",
        paragraph: "Schedule a consultation with our expert ophthalmology team",
    ...
    km: {
      title: "សេវាកម្មជំងឺភ្នែក",
      subtitle:
        "ការថែទាំភ្នែកទូលំទូលាយជាមួយការធ្វើរោគវិនិច្ឆ័យ និងការព្យាបាលឆ្នើម",
      tagline: "ការថែទាំភ្នែកដោយអ្នកជំនាញ",
      stats: {
      ...
      cta: {
        title: "ទិដ្ឋភាពរបស់អ្នកសមនឹងទទួលបានល្អបំផុត",
        description: "កក់ណាត់ពិគ្រោះជាមួយក្រុមអ្នកជំនា៍ជំងឺភ្នែករបស់យើង",
        paragraph: "កក់ណាត់ពិគ្រោះជាមួយក្រុមអ្នកជំនាញជំងឺភ្នែករបស់យើង",
        button: "កក់ណាត់",
      },
      error: {
  ...
  useEffect(() => {
    const fetchServices = async () => {
      ...
      try {
        ...
        console.error("Error fetching Ophthalmology services:", err);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-16 md:mb-20 max-w-6xl">
            ...
            >
                       {" "}
              {currentLanguage === "en"
                ? "The Ophthalmology Department located in Khmer-Soviet Friendship Hospital, provides a wide range of services for eye conditions, from routine eye examinations to advanced surgical treatments. Our comprehensive ophthalmology services include:."
                : "នាយកដ្ឋានជំងឺភ្នែក មានទីតាំងនៅក្នុងមន្ទីរពេទ្យមិត្តភាពខ្មែរ-សូវៀត ផ្តល់សេវាកម្មយ៉ាងទូលំទូលាយសម្រាប់ជំងឺភ្នែក តាំងពីការពិនិត្យភ្នែកធម្មតា ដល់ការព្យាបាលតាមរយៈការវះកាត់កម្រិតខ្ពស់។ សេវាកម្មជំងឺភ្នែកទូលំទូលាយរបស់យើងរួមមាន:"}
                     {" "}
            </p>
